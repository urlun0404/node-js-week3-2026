...
function filterByQuery(list, query) {
  return query.level ? list.filter((m) => m.level === query.level) : list;
}
...
function hasRequiredFields(body) {
  ...
  return (
    requiredFields.filter((field) => field in body && body[field] != null)
      .length === requiredFields.length
  );
}
function validateBody(body) {
  if (!body || typeof body !== "object" || !hasRequiredFields(body)) {
